...
export function getDashboard(payload: { plant_id?: number } = {}): DashboardData {
  const d = getDb()
  ...
  const rawTotal = num(
    d.prepare(`SELECT COALESCE(SUM(change_qty),0) AS q FROM stock_movements m WHERE material_type='raw'${mAnd}`).get() as { q: number }
  )
  const rawByPlant = d
    .prepare(
  ...
  const finishedTotal = num(
    d.prepare(`SELECT COALESCE(SUM(change_qty),0) AS q FROM stock_movements m WHERE material_type='finished'${mAnd}`).get() as { q: number }
  )
  const finishedByPlant = d
    .prepare(
      `SELECT m.plant_id, p.name AS plant_name, ROUND(COALESCE(SUM(m.change_qty),0),3) AS qty
       FROM stock_movements m JOIN plants p ON p.id = m.plant_id
  ...
  const totalPurchased = num(
    d.prepare(`SELECT COALESCE(SUM(quantity),0) AS q FROM purchases${plWhere}`).get() as { q: number }
  )
  const totalConsumed = num(
    d.prepare(`SELECT COALESCE(SUM(raw_qty),0) AS q FROM productions${plWhere}`).get() as { q: number }
  )
  const totalProduced = num(
    d.prepare(`SELECT COALESCE(SUM(change_qty),0) AS q FROM stock_movements m WHERE type='production_output'${mAnd}`).get() as { q: number }
  )
  const totalDispatched = num(
    d.prepare(`SELECT COALESCE(SUM(qty_cm),0) AS q FROM dispatches${plWhere}`).get() as { q: number }
  )

  const pendingSupplierPayment = money(
    d.prepare(`SELECT COALESCE(SUM(COALESCE(amount,0) - paid_amount),0) AS q FROM purchases WHERE payment_status <> 'paid'${plAnd}`).get() as { q: number }
  )
  const pendingDeliveries = (
    d.prepare(`SELECT COUNT(*) AS q FROM dispatches WHERE delivery_status='pending'${plAnd}`).get() as { q: number }
  ).q
  const deliveredNoRate = (
    d.prepare(`SELECT COUNT(*) AS q FROM dispatches WHERE delivery_status='delivered' AND rate IS NULL${plAnd}`).get() as { q: number }
  ).q
  ...
  const openRacks = (
    d.prepare(`SELECT COUNT(*) AS q FROM racks WHERE status <> 'closed'`).get() as { q: number }
  ).q
  const rackSalesAmount = money(
    d.prepare(`SELECT COALESCE(SUM(amount),0) AS q FROM rack_sales`).get() as { q: number }
  )
  ...
  const totalRackExpenses = money(
    d.prepare(`SELECT COALESCE(SUM(amount),0) AS q FROM rack_expenses`).get() as { q: number }
  )
  ...
  const counts = {
    plants: (d.prepare(`SELECT COUNT(*) AS q FROM plants`).get() as { q: number }).q,
    suppliers: (d.prepare(`SELECT COUNT(*) AS q FROM suppliers`).get() as { q: number }).q,
    customers: (d.prepare(`SELECT COUNT(*) AS q FROM customers`).get() as { q: number }).q,
    transporters: (d.prepare(`SELECT COUNT(*) AS q FROM transporters`).get() as { q: number }).q,
    companies: (d.prepare(`SELECT COUNT(*) AS q FROM companies`).get() as { q: number }).q,
    racks: (d.prepare(`SELECT COUNT(*) AS q FROM racks`).get() as { q: number }).q
  }
