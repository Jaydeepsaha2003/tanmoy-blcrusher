...
export function listProductions(filter: ProductionFilter = {}): Production[] {
  const d = getDb()
  const where: string[] = []
...
export function previewProduction(payload: {
  plant_id: number
  raw_qty: number
}): { product_name: string; percentage: number; quantity: number }[] {
  const d = getDb()
  const settings = d
...
export function createProduction(p: {
  ...
  const available = rawLocationBalance(d, p.stock_location_id)
  ...
  const tx = d.transaction(() => {
    const no = nextNumber('PROD', 'production')
    const info = d
      .prepare(
    ...
    for (const s of settings) {
      ...
      outStmt.run(productionId, s.product_name, s.output_percentage, qty)
      if (qty > 0) {
        addMovement(d, {
  ...
  const id = tx()
  return listProductions().find((x) => x.id === id) as Production
}

export function deleteProduction(payload: { id: number }): { ok: boolean; error?: string } {
  const d = getDb()
  const prod = d.prepare(`SELECT * FROM productions WHERE id = ?`).get(payload.id) as Production
  ...
  try {
    const tx = d.transaction(() => {
      ...
      d.prepare(`DELETE FROM stock_movements WHERE ref_no=? AND type='production_output'`).run(
        prod.production_no
      )
      for (const o of outputs) {
        if (finishedBalance(d, prod.plant_id, o.product_name) < 0)
          throw new Error(
            `Cannot delete: ${o.product_name} produced here has already been dispatched.`
          )
      }
      // Restore raw material.
      d.prepare(`DELETE FROM stock_movements WHERE ref_no=? AND type='production_consume'`).run(
        prod.production_no
      )
      d.prepare(`DELETE FROM production_outputs WHERE production_id = ?`).run(payload.id)
      d.prepare(`DELETE FROM productions WHERE id = ?`).run(payload.id)
    })
    tx()
    return { ok: true }
  } catch (e) {
