...
function parseModuleList(raw: string | null | undefined): ModuleKey[] {
  try {
    const arr = JSON.parse(raw || '[]')
    return Array.isArray(arr)
      ? arr.filter((m): m is ModuleKey => VALID_MODULES.has(m as ModuleKey))
      : []
  } catch {
    return []
  }
...
function sanitizeModules(modules: unknown): ModuleKey[] {
  if (!Array.isArray(modules)) return []
  return modules.filter((m): m is ModuleKey => typeof m === 'string' && VALID_MODULES.has(m as ModuleKey))
}

/** Build stored access from UI input: view = modules ∪ edit; edit ⊆ view; admin = all. */
function buildAccess(
  role: Role,
  modules: ModuleKey[],
  editModules: ModuleKey[]
): { modules: ModuleKey[]; editModules: ModuleKey[]; accessLevel: AccessLevel } {
  if (role === 'admin') return { modules: [], editModules: [], accessLevel: 'edit' }
  const edit = sanitizeModules(editModules)
  const view = Array.from(new Set([...sanitizeModules(modules), ...edit]))
...
export async function getUserById(id: number): Promise<User | null> {
  const d = getDb()
  const row = await d.prepare(`SELECT * FROM users WHERE id = ? AND active = 1`).get(id) as
    | UserRow
    | undefined
...
export async function updateUser(p: UserInput): Promise<User> {
  const d = getDb()
  if (!p.id) throw new Error('Missing user id.')
  const old = await d.prepare(`SELECT * FROM users WHERE id = ?`).get(p.id) as UserRow | undefined
...
export async function deleteUser(payload: { id: number }): Promise<{ ok: boolean; error?: string }> {
  const d = getDb()
  const me = getCurrentUser()
  if (me && me.id === payload.id) return { ok: false, error: 'You cannot delete your own account.' }
  const row = await d.prepare(`SELECT * FROM users WHERE id = ?`).get(payload.id) as UserRow | undefined
...
export async function changeOwnPassword(payload: { current: string; next: string }): Promise<{
  ok: boolean
  error?: string
}> {
  const d = getDb()
  ...
  const row = await d.prepare(`SELECT * FROM users WHERE id = ?`).get(me.id) as UserRow | undefined
